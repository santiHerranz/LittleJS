...
class TowerWeapon extends EngineObject {
    constructor(pos, parent) {
        //        super(pos, vec2(.6), 2, vec2(8));
        super(pos, vec2(.6,.8), 3, vec2(16,16));
    ...
    update() {
        ...
        if (this.triggerIsDown) {
            ...
            for (; this.fireTimeBuffer > 0; this.fireTimeBuffer -= 1 / this.fireRate) {
                ...
                this.recoilTimer.set(2); // slow recoil
                const direction = vec2(0, .3).rotate(-this.angle); //vec2(this.getMirrorSign(this.bulletSpeed), 0);
                const velocity = direction.rotate(rand(1, -1) * this.bulletSpread);
                const bullet = new TowerBullet(this.pos, this.parent, velocity, this.damage);
...
class TowerBullet extends EngineObject {
